# Extracted provenance cell for kwnn_2.pdf
# Source notebook: Traditional_KWNN_passive.ipynb
# Source cell index: 5
...
plt.rcParams.update({
    ...
    "axes.linewidth": 0.3  # Global border thickness
})

# Prepare figure
fig = plt.figure(figsize=(3.4, 2.2), dpi=500)

# Create a 2-row grid: top for timing, bottom for ECDF
fig, ax1 = plt.subplots(figsize=(3.4, 2.4), dpi=500)

# ---- BOTTOM PLOT: ECDF + INSET BOX ----
line_styles = [':', '--', '-.', '-']
line_width = 1.0
k_values = sorted(comparison_df['k'].unique())
...
# inset_timing.set_facecolor("white")
# inset_timing.set_zorder(10)  # Keeps it above the main plot’s grid
...
# Colored bars to match box fills
...
inset_ax.set_zorder(10)  # Keeps it above the main plot’s grid

data_to_plot = [comparison_df[comparison_df['k'] == k]["Error_Euclidean"] for k in k_values]
